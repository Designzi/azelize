...
Le **système fruit / 17 combinaisons** (`docs/familles/charte-couleur.md`, `menthe/fraise/miel/citron/kiwi`) est **superseded** : `lib/accents.ts` renvoie désormais `brand` pour tout accent ; les tokens fruit subsistent mais ne doivent plus être utilisés (élagage à venir). Le site est **monochrome** (marque + neutres + jaune ponctuel). Jamais `ok`/`warn`/`danger` en surface, jamais de couleur en dur (`check:tokens`).
