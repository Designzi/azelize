...
> **Rappel de cadrage.** On ne construit pas l'architecture (elle existe). On **porte ~40 pages de contenu** en entrées de collection + registre, et on factorise quelques patterns visuels. **Ce document ne déclenche aucune migration** — il la prépare ; chaque lot sera validé séparément.
...
### Lot 1 — Réconciliation tokens (Ocean Twilight) + recâblage
- **Périmètre** :
...
### Lot 2 — Données socles, prix & méga-menu
...
### Lot 3 — Hubs services C1 (enrichissement)
...
### Lot 4 — Pages C0 statiques manquantes
...
### Lot 5 — Collections + routes dynamiques
- **5a Guides** : collection `guides` + `/guides` + `/guides/[slug]` (`articleJsonLd`) ; porter les 7 guides.
- **5b Métiers** : porter les métiers manquants en MDX `sections[]` (contenu **unique** par métier) ; passer `statut` à `ship` au cas par cas.
- **5c Villes** : collection `villes` + `/{service}/[ville]` gaté par `MATRICE_VILLE` ; **rester `conditionnel`** tant qu'il manque cas local + FAQ locale (ne rien shipper sans contenu réel). **Créer aussi la route triple gatée** `/{service}/[metier]/[ville].astro`, statut **`exclu`** par défaut (structure prête, aucune page générée).
- **5d Avis** : collection `temoignages` + `/avis` (bloc `testimonial`).
- **5e Glossaire** : collection `glossaire` + `/glossaire` (filtre alpha `<script>`).
...
*Fin du plan. Aucune migration exécutée ; document de préparation uniquement.*
